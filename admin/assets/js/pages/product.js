function editProduct(productId) {
  ...
  $.get(
    "./logic/product.php?act=getproduct&id=" + productId,
    function (response) {
      $.get("./view/pages/products/product-form.php", function (reponseHtml) {
        ...
        productForm.elements["addproductbtn"].setAttribute(
          "name",
          "editproductbtn"
        );
        ...
        $("#cartModal #product-action-btn").click(function (e) {
          ...
          formData.append(
            "mo_ta",
            CKEDITOR.instances.descriptionProductEditor.getData()
          );
          formData.append(
            "thong_tin",
            CKEDITOR.instances.infoProductEditor.getData()
          );
          // return;
          $.ajax({
            type: "POST",
  ...
  $.get(
    ...
    function (response) {
      $.get("./view/pages/products/product-form.php", function (reponseHtml) {
        ...
        $("#cartModal .modal-body").html(reponseHtml);
        $("#cartModal #cartModalLabel").text(
          `Xem chi tiết sản phẩm #${productId}`
        );
        const productForm = document.getElementById("product-form");
        console.log(productForm.elements);
...
function deleteProduct(btnElement, productId) {
  ...
  alertModal(
    `Bạn có muốn xóa sản phẩm #${productId}  này ?`,
    "Chọn tiếp tục để xóa, chọn đóng để trở lại"
  );
...
function updateReplyReview(idReview, idUser, idReply) {
  ...
  $.ajax({
    ...
    success: function (response) {
      const {
        status,
        content: { content, date_modified, id_reply, id_review, id_user },
      } = JSON.parse(response);

      $("#cartModalBtn").trigger("click");
      $("#cartModalLabel").text(`Trả lời Review #${idReview}`);
      $("#cartModal .modal-body").html(
        `<form id="reply-review-form" onsubmit="updateReplyReview()" action=""><div className="form-group"><label htmlFor="">Nội dung bình luận</label><textarea  class="form-control" readonly>${reviewContent}</textarea></div> <div class="form-group mt-5"><label htmlFor="">Trả lời bình luận</label><textarea class="form-control" id="replyContent" placeholder="Bình luận ở đây!">${content}</textarea></div></form>`
      );
      $("#cartModal .action-btn").removeClass("d-none");
