  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      if (logicType == "buynow") {
        ...
        $("#cartModalLabel").text(
          `Đã thêm sản phẩm ${tensp} vào danh sách yêu thích`
        );
        $("#cartModal .modal-body").text(
          `Đã thêm sản phẩm ${tensp} vào danh sách yêu thích, Bạn có muốn vào xem danh sách yêu thích hay không ?`
        );
        ...
        if (status == 1) {
          ...
          $("#cartModal .modal-body").text(
            `Đã thêm sản phẩm ${tensp} vào giỏ hàng, Bạn có muốn vào xem giỏ hàng hay không ?`
          );
        } else if (status == 0) {
          // Show Error message
          $("#cartModalLabel").text(`Vượt quá tồn kho`);
...
function handleDeleteCart(idcart) {
  ...
  console.log(
    "tensp: ",
    event.currentTarget.firstElementChild.getAttribute("data-name")
  );
  ...
  $("#cartModal .modal-body").text(
    `Bạn có muốn xóa sản phẩm ${tensp} giỏ hàng hay không ? Có chọn tiếp tục không chọn đóng`
  );
    ...
    $.ajax({
      ...
      success: function (response, statusText) {
        ...
        $.get("./logic/shopping-cart-content.php", function (reponseHtml) {
          ...
          $("#notify-update-cart").text(
            `Sản phẩm ${tensp} đã được xóa khỏi giỏ hàng!`
          );
        });
...
function handleDeleteWishlist(idWishlist) {
  ...
  $("#cartModal .modal-body").text(
    `Bạn có muốn xóa sản phẩm ${tensp} danh sách yêu thích hay không ? Có chọn tiếp tục không chọn đóng`
  );
...
const zoomProductDetail = () => {
  ...
  [...zoomProductBtns].forEach((btn) => {
    btn.addEventListener("click", (e) => {
      ...
      // console.log('productModal: ', productModal)
      const modalImgElement = productModal.querySelector(
        ".main-image.images > img"
      );
      const modalProductnameElement =
        productModal.querySelector(".product-info > h1");
      const modalProductpriceElement = productModal.querySelector(".new-price");
      const modalProductoldpriceElement =
        productModal.querySelector(".old-price");
      ...
      modalProductpriceElement.textContent = new_price.toLocaleString("vi-VN", {
        style: "currency",
        currency: "VND",
      });
      modalProductQtyElement.value = sl;
      modalProductDescElement.innerHTML = mo_ta;
      modalProductoldpriceElement.textContent = don_gia.toLocaleString(
        "vi-VN",
        { style: "currency", currency: "VND" }
      );
      modalProductSeeAllElement.setAttribute(
        "href",
        "./index.php?act=detailproduct&id=" + productId
      );
      // modalProductDescElement.textContent =

      const formAddToCart = productModal.querySelector(
      ...
      formCartElements["addtocartbtn"].setAttribute(
        "onclick",
        "handleAddCart('addtocart', 'addcart')"
      );
...
function selectProvince(currentProvince) {
  ...
  const provinceName =
    checkoutForm.elements["province-select"].options[
      checkoutForm.elements["province-select"].selectedIndex
    ].text;
...
function selectDistrict(currentDistrict) {
  ...
  const districtName =
    checkoutForm.elements["district-select"].options[
      checkoutForm.elements["district-select"].selectedIndex
    ].text;
...
function selectWard(currentWard) {
  ...
  const wardName =
    checkoutForm.elements["ward-select"].options[
      checkoutForm.elements["ward-select"].selectedIndex
    ].text;
...
function calcShippingFee(
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      const {
        data: { total: shippingFee, insurance_fee },
      } = response;
